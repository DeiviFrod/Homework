...
join employees e on e.id = re.employee_id 
join roles1 r on r.id = re.role_id
--where role_name like '%Java %';  фильтр по слову
where role_id in (4,5,6); -- по id

-- 8. Вывести имена и должность только Python разработчиков.
select employee_name, role_name from roles_employee re 
join roles1 r on r.id = re.role_id
join employees e on e.id = re.employee_id 
--where role_name like '%Python%';    фильтр по слову
where role_id in (1,2,3); --фильтр по id роли
